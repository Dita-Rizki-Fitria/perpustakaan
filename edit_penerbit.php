...
if (isset($_GET['id'])) {
    ...
    // Fetch the penerbit data by ID
    $penerbitData = $penerbitCRUD->getPenerbitById($id);

    // Check if penerbit data is found
    if ($penerbitData) {
        // Assign values to variables for use in the HTML form
        $nama = $penerbitData['nama'];
        $alamat = $penerbitData['alamat'];
        $no_telp = $penerbitData['no_telp'];
    } else {
        // Redirect or handle the case where no penerbit is found
        header("Location: ../penerbit.php");
        exit();
    }
} else {
    // Redirect or handle the case where 'id' parameter is not present
    header("Location: ../penerbit.php");
    exit();
}
?>
        ...
        <div class="text-dark p-4 col-4">
            ...
            <div class="conten">
                <div class="text-dark p-5 ">
                    ...
                    <div class="rounded p-2 alert alert-secondary">
                        <form action="php/penerbit.php" method="post">

                            <!-- Tambahkan kolom input tersembunyi untuk update_id -->
                            <input type="hidden" name="update_id" value="<?php echo $id; ?>">

                            <div class="form-group text-dark   ">
                                <label for="" class="control-label">Nama</label>
                                <input type="text" class="form-control form-control-sm" name="nama" value="<?php echo $nama; ?>" required>
                            </div>
                            <div class="form-group  text-dark">
                                <label for="" class="control-label">Alamat</label>
                                <input type="text" class="form-control form-control-sm" name="alamat" value="<?php echo $alamat; ?>" required>
                            </div>
                            <div class="form-group  text-dark">
                                <label for="" class="control-label">No Handphone</label>
                                <input type="text" class="form-control form-control-sm" name="no_telp" value="<?php echo $no_telp; ?>" required>
                            </div>
                                ...
                                <div class="modal-dialog modal-dialog-centered" role="document">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <em>
                                                <h5 class="modal-title" id="modalSayaLabel">Edit Penerbit</h5>
                                            </em>
                                        ...
                                        <div class="modal-body">
                                            <strong>Apakah Anda yakin ingin menyimpan pembaruan data ini ?</strong>
                                        </div>
